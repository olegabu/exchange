...
#ifndef _EXCHANGE_ADDINSTRUMENT_CXX_H_
#define _EXCHANGE_ADDINSTRUMENT_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(92);
    ...
    AddInstrument(char *buffer, const std::uint64_t bufferLength) :
        AddInstrument(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    AddInstrument(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        AddInstrument(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    AddInstrument &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    AddInstrument &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        AddInstrument skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    AddInstrument &sessionId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    AddInstrument &senderCompId(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    AddInstrument &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AddInstrument &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    AddInstrument &putSenderCompId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    AddInstrument &putSenderCompId(const std::string_view str)
    {
    ...
    #else
    AddInstrument &putSenderCompId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    AddInstrument &account(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AddInstrument &putAccount(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    AddInstrument &putAccount(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putAccount(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    AddInstrument &putAccount(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #else
    AddInstrument &putAccount(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    AddInstrument &clOrdId(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AddInstrument &putClOrdId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    AddInstrument &putClOrdId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    AddInstrument &putClOrdId(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    #else
    AddInstrument &putClOrdId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    AddInstrument &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AddInstrument &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    AddInstrument &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddInstrument &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    AddInstrument &putSymbol(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #else
    AddInstrument &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    AddInstrument &tickSize(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    AddInstrument &lotSize(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    AddInstrument &maxOrderQty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const AddInstrument &_writer)
{
    AddInstrument writer(
        _writer.m_buffer,
        _writer.m_offset,
