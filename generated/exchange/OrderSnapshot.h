...
#ifndef _EXCHANGE_ORDERSNAPSHOT_CXX_H_
#define _EXCHANGE_ORDERSNAPSHOT_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(123);
    ...
    OrderSnapshot(char *buffer, const std::uint64_t bufferLength) :
        OrderSnapshot(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderSnapshot(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderSnapshot(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderSnapshot &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderSnapshot &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderSnapshot skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    OrderSnapshot &orderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderSnapshot &sessionId(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    OrderSnapshot &senderCompId(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    OrderSnapshot &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderSnapshot &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderSnapshot &putSenderCompId(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 16, src, sizeof(char) * 16);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderSnapshot &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderSnapshot &putSenderCompId(const std::string_view str)
    {
    ...
    #else
    OrderSnapshot &putSenderCompId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    OrderSnapshot &account(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderSnapshot &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 16, "array too large for putAccount");

        if (N > 0)
        {
            std::memcpy(m_buffer + m_offset + 32, src.data(), sizeof(char) * N);
        }
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderSnapshot &putAccount(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderSnapshot &putAccount(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 32, src, sizeof(char) * 16);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderSnapshot &putAccount(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderSnapshot &putAccount(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #else
    OrderSnapshot &putAccount(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    OrderSnapshot &clOrdId(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderSnapshot &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 20, "array too large for putClOrdId");

        if (N > 0)
        {
            std::memcpy(m_buffer + m_offset + 48, src.data(), sizeof(char) * N);
        }
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderSnapshot &putClOrdId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderSnapshot &putClOrdId(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 48, src, sizeof(char) * 20);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderSnapshot &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderSnapshot &putClOrdId(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    #else
    OrderSnapshot &putClOrdId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    OrderSnapshot &instrumentId(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    OrderSnapshot &side(const Side::Value value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD std::uint8_t ordTypeRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    OrderSnapshot &ordType(const OrdType::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    OrderSnapshot &timeInForce(const TimeInForce::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    OrderSnapshot &price(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    OrderSnapshot &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    {
        std::int64_t val;
        std::memcpy(&val, m_buffer + m_offset + 91, sizeof(std::int64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    OrderSnapshot &cumQty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 91, &val, sizeof(std::int64_t));
        return *this;
    }
    ...
    {
        std::int64_t val;
        std::memcpy(&val, m_buffer + m_offset + 99, sizeof(std::int64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    OrderSnapshot &leavesQty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 99, &val, sizeof(std::int64_t));
        return *this;
    }

    SBE_NODISCARD static const char *cumNotionalMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t cumNotionalId() SBE_NOEXCEPT
    {
        return 14;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t cumNotionalSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool cumNotionalInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t cumNotionalEncodingOffset() SBE_NOEXCEPT
    {
        return 107;
    }

private:
    Int128 m_cumNotional;

public:
    SBE_NODISCARD Int128 &cumNotional()
    {
        m_cumNotional.wrap(m_buffer, m_offset + 107, m_actingVersion, m_bufferLength);
        return m_cumNotional;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderSnapshot &_writer)
{
    OrderSnapshot writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("cumNotional": )";
    builder << writer.cumNotional();
