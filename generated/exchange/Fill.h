...
#ifndef _EXCHANGE_FILL_CXX_H_
#define _EXCHANGE_FILL_CXX_H_
...
public:
    ...
    Fill(char *buffer, const std::uint64_t bufferLength) :
        Fill(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Fill(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Fill(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Fill &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Fill &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Fill skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Fill &instrumentId(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    Fill &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    Fill &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Fill &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    Fill &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Fill &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    Fill &putSymbol(const std::string_view str)
    {
    ...
    #else
    Fill &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #pragma GCC diagnostic pop
        ...
        Executions &sessionId(const std::uint64_t value) SBE_NOEXCEPT
        {
        ...
        Executions &senderCompId(const std::uint64_t index, const char value)
        {
            if (index >= 16)
            {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Executions &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Executions &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        Executions &putSenderCompId(const char *const src) SBE_NOEXCEPT
        #endif
        {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Executions &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_STRING_VIEW
        Executions &putSenderCompId(const std::string_view str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 16)
            {
        ...
        #else
        Executions &putSenderCompId(const std::string &str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 16)
            {
        ...
        Executions &account(const std::uint64_t index, const char value)
        {
            if (index >= 16)
            {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Executions &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Executions &putAccount(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        Executions &putAccount(const char *const src) SBE_NOEXCEPT
        #endif
        {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Executions &putAccount(const char (&src)[N]) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_STRING_VIEW
        Executions &putAccount(const std::string_view str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 16)
            {
        ...
        #else
        Executions &putAccount(const std::string &str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 16)
            {
        ...
        Executions &clOrdId(const std::uint64_t index, const char value)
        {
            if (index >= 20)
            {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Executions &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Executions &putClOrdId(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        Executions &putClOrdId(const char *const src) SBE_NOEXCEPT
        #endif
        {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Executions &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_STRING_VIEW
        Executions &putClOrdId(const std::string_view str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 20)
            {
        ...
        #else
        Executions &putClOrdId(const std::string &str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 20)
            {
        ...
        Executions &orderId(const std::uint64_t value) SBE_NOEXCEPT
        {
            std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Executions &side(const Side::Value value) SBE_NOEXCEPT
        {
        ...
        static SBE_CONSTEXPR std::int64_t lastPxNullValue() SBE_NOEXCEPT
        {
            return SBE_NULLVALUE_INT64;
        }

        static SBE_CONSTEXPR std::int64_t lastPxMinValue() SBE_NOEXCEPT
        {
            return INT64_C(-9223372036854775807);
        }

        static SBE_CONSTEXPR std::int64_t lastPxMaxValue() SBE_NOEXCEPT
        {
            return INT64_C(9223372036854775807);
        }

        static SBE_CONSTEXPR std::size_t lastPxEncodingLength() SBE_NOEXCEPT
        {
            return 8;
        }

        SBE_NODISCARD std::int64_t lastPx() const SBE_NOEXCEPT
        {
            std::int64_t val;
            std::memcpy(&val, m_buffer + m_offset + 69, sizeof(std::int64_t));
            return SBE_LITTLE_ENDIAN_ENCODE_64(val);
        }

        Executions &lastPx(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
            std::memcpy(m_buffer + m_offset + 69, &val, sizeof(std::int64_t));
            return *this;
        }
        ...
        {
            std::int64_t val;
            std::memcpy(&val, m_buffer + m_offset + 77, sizeof(std::int64_t));
            return SBE_LITTLE_ENDIAN_ENCODE_64(val);
        }

        Executions &lastQty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
            std::memcpy(m_buffer + m_offset + 77, &val, sizeof(std::int64_t));
            return *this;
        }
        ...
        {
            std::int64_t val;
            std::memcpy(&val, m_buffer + m_offset + 85, sizeof(std::int64_t));
            return SBE_LITTLE_ENDIAN_ENCODE_64(val);
        }

        Executions &leavesQty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
            std::memcpy(m_buffer + m_offset + 85, &val, sizeof(std::int64_t));
            return *this;
        }
        ...
        {
            std::int64_t val;
            std::memcpy(&val, m_buffer + m_offset + 93, sizeof(std::int64_t));
            return SBE_LITTLE_ENDIAN_ENCODE_64(val);
        }

        Executions &cumQty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
            std::memcpy(m_buffer + m_offset + 93, &val, sizeof(std::int64_t));
            return *this;
        }

        SBE_NODISCARD static const char *avgPxMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
        {
            switch (metaAttribute)
            {
                case MetaAttribute::PRESENCE: return "required";
        ...
        static SBE_CONSTEXPR std::uint16_t avgPxId() SBE_NOEXCEPT
        {
            return 21;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t avgPxSinceVersion() SBE_NOEXCEPT
        {
            return 0;
        }

        SBE_NODISCARD bool avgPxInActingVersion() SBE_NOEXCEPT
        {
            return true;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::size_t avgPxEncodingOffset() SBE_NOEXCEPT
        {
            return 101;
        }

        static SBE_CONSTEXPR std::int64_t avgPxNullValue() SBE_NOEXCEPT
        {
            return SBE_NULLVALUE_INT64;
        }

        static SBE_CONSTEXPR std::int64_t avgPxMinValue() SBE_NOEXCEPT
        {
            return INT64_C(-9223372036854775807);
        }

        static SBE_CONSTEXPR std::int64_t avgPxMaxValue() SBE_NOEXCEPT
        {
            return INT64_C(9223372036854775807);
        }

        static SBE_CONSTEXPR std::size_t avgPxEncodingLength() SBE_NOEXCEPT
        {
            return 8;
        }

        SBE_NODISCARD std::int64_t avgPx() const SBE_NOEXCEPT
        {
            std::int64_t val;
        ...
        Executions &avgPx(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        static SBE_CONSTEXPR std::uint64_t counterpartyOrderIdNullValue() SBE_NOEXCEPT
        {
            return SBE_NULLVALUE_UINT64;
        }

        static SBE_CONSTEXPR std::uint64_t counterpartyOrderIdMinValue() SBE_NOEXCEPT
        {
            return UINT64_C(0x0);
        }

        static SBE_CONSTEXPR std::uint64_t counterpartyOrderIdMaxValue() SBE_NOEXCEPT
        {
            return UINT64_C(0xfffffffffffffffe);
        }
        ...
        SBE_NODISCARD std::uint64_t counterpartyOrderId() const SBE_NOEXCEPT
        {
            std::uint64_t val;
            std::memcpy(&val, m_buffer + m_offset + 109, sizeof(std::uint64_t));
            return SBE_LITTLE_ENDIAN_ENCODE_64(val);
        }

        Executions &counterpartyOrderId(const std::uint64_t value) SBE_NOEXCEPT
        {
            std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
            std::memcpy(m_buffer + m_offset + 109, &val, sizeof(std::uint64_t));
            return *this;
        }

        SBE_NODISCARD static const char *aggressorMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
        {
            switch (metaAttribute)
            {
                case MetaAttribute::PRESENCE: return "required";
        ...
        static SBE_CONSTEXPR std::uint16_t aggressorId() SBE_NOEXCEPT
        {
            return 23;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t aggressorSinceVersion() SBE_NOEXCEPT
        {
            return 0;
        }

        SBE_NODISCARD bool aggressorInActingVersion() SBE_NOEXCEPT
        {
            return true;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::size_t aggressorEncodingOffset() SBE_NOEXCEPT
        {
            return 117;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::size_t aggressorEncodingLength() SBE_NOEXCEPT
        {
            return 1;
        }

        SBE_NODISCARD std::uint8_t aggressorRaw() const SBE_NOEXCEPT
        {
            std::uint8_t val;
        ...
        SBE_NODISCARD Bool::Value aggressor() const
        {
            std::uint8_t val;
            std::memcpy(&val, m_buffer + m_offset + 117, sizeof(std::uint8_t));
            return Bool::get((val));
        ...
        Executions &aggressor(const Bool::Value value) SBE_NOEXCEPT
        {
            std::uint8_t val = (value);
        ...
        friend std::basic_ostream<CharT, Traits> & operator << (
            std::basic_ostream<CharT, Traits> &builder, Executions &writer)
        {
            builder << '{';
            builder << R"("sessionId": )";
...
public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t executionsId() SBE_NOEXCEPT
    {
        return 10;
    }

    SBE_NODISCARD inline Executions &executions()
    {
        m_executions.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_executions;
    }

    Executions &executionsCount(const std::uint16_t count)
    {
        m_executions.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_executions;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t executionsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool executionsInActingVersion() const SBE_NOEXCEPT
    {
        return true;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Fill &_writer)
{
    Fill writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    {
        ...
        builder << R"("executions": [)";
        writer.executions().forEach(
...
void skip()
{
    auto &executionsGroup { executions() };
    while (executionsGroup.hasNext())
    {
        executionsGroup.next().skip();
    }
}
...
SBE_NODISCARD static std::size_t computeLength(std::size_t executionsLength = 0)
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += Executions::sbeHeaderSize();
    if (executionsLength > 65534LL)
    {
        throw std::runtime_error("executionsLength outside of allowed range [E110]");
    }
    length += executionsLength *Executions::sbeBlockLength();
