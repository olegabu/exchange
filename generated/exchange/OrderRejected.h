...
#ifndef _EXCHANGE_ORDERREJECTED_CXX_H_
#define _EXCHANGE_ORDERREJECTED_CXX_H_
...
public:
    ...
    OrderRejected &sessionId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderRejected &senderCompId(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    OrderRejected &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderRejected &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderRejected &putSenderCompId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderRejected &putSenderCompId(const std::string_view str)
    {
    ...
    #else
    OrderRejected &putSenderCompId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    OrderRejected &account(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderRejected &putAccount(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderRejected &putAccount(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putAccount(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderRejected &putAccount(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #else
    OrderRejected &putAccount(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    OrderRejected &clOrdId(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderRejected &putClOrdId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderRejected &putClOrdId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderRejected &putClOrdId(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    #else
    OrderRejected &putClOrdId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    OrderRejected &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderRejected &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderRejected &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderRejected &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderRejected &putSymbol(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #else
    OrderRejected &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    OrderRejected &side(const Side::Value value) SBE_NOEXCEPT
    {
    ...
    OrderRejected &ordType(const OrdType::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    OrderRejected &timeInForce(const TimeInForce::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    {
        std::int64_t val;
        std::memcpy(&val, m_buffer + m_offset + 71, sizeof(std::int64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    OrderRejected &price(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 71, &val, sizeof(std::int64_t));
        return *this;
    }
    ...
    {
        std::int64_t val;
        std::memcpy(&val, m_buffer + m_offset + 79, sizeof(std::int64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    OrderRejected &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 79, &val, sizeof(std::int64_t));
        return *this;
    }
