...
#ifndef _EXCHANGE_INSTRUMENTADDED_CXX_H_
#define _EXCHANGE_INSTRUMENTADDED_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(96);
    ...
    InstrumentAdded(char *buffer, const std::uint64_t bufferLength) :
        InstrumentAdded(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    InstrumentAdded(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        InstrumentAdded(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    InstrumentAdded &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    InstrumentAdded &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        InstrumentAdded skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    InstrumentAdded &sessionId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    InstrumentAdded &senderCompId(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    InstrumentAdded &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentAdded &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentAdded &putSenderCompId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentAdded &putSenderCompId(const std::string_view str)
    {
    ...
    #else
    InstrumentAdded &putSenderCompId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    InstrumentAdded &account(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentAdded &putAccount(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentAdded &putAccount(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putAccount(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentAdded &putAccount(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #else
    InstrumentAdded &putAccount(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    InstrumentAdded &clOrdId(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentAdded &putClOrdId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentAdded &putClOrdId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentAdded &putClOrdId(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    #else
    InstrumentAdded &putClOrdId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    InstrumentAdded &instrumentId(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD char symbol(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 64 + (index * 1), sizeof(char));
        return (val);
    }

    InstrumentAdded &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 64 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getSymbol(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 64, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 8, "array too large for putSymbol");

        if (N > 0)
        {
            std::memcpy(m_buffer + m_offset + 64, src.data(), sizeof(char) * N);
        }
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentAdded &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentAdded &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 64, src, sizeof(char) * 8);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentAdded &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentAdded &putSymbol(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
        ...
        if (srcLength > 0)
        {
            std::memcpy(m_buffer + m_offset + 64, str.data(), srcLength);
        }
    ...
    #else
    InstrumentAdded &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
        ...
        std::memcpy(m_buffer + m_offset + 64, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 8; ++start)
        {
            m_buffer[m_offset + 64 + start] = 0;
        }
    ...
    InstrumentAdded &tickSize(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    InstrumentAdded &lotSize(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    InstrumentAdded &maxOrderQty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const InstrumentAdded &_writer)
{
    InstrumentAdded writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "InstrumentAdded", )";
