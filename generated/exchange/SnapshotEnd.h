...
#ifndef _EXCHANGE_SNAPSHOTEND_CXX_H_
#define _EXCHANGE_SNAPSHOTEND_CXX_H_
...
public:
    ...
    SnapshotEnd(char *buffer, const std::uint64_t bufferLength) :
        SnapshotEnd(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SnapshotEnd(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SnapshotEnd(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SnapshotEnd &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SnapshotEnd &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SnapshotEnd skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SnapshotEnd &orderCount(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SnapshotEnd &_writer)
{
    SnapshotEnd writer(
        _writer.m_buffer,
        _writer.m_offset,
