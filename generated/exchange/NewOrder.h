...
#ifndef _EXCHANGE_NEWORDER_CXX_H_
#define _EXCHANGE_NEWORDER_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(87);
    ...
    NewOrder &sessionId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    NewOrder &senderCompId(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    NewOrder &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    NewOrder &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    NewOrder &putSenderCompId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    NewOrder &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    NewOrder &putSenderCompId(const std::string_view str)
    {
    ...
    #else
    NewOrder &putSenderCompId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    NewOrder &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    NewOrder &putAccount(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    NewOrder &putAccount(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    NewOrder &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    NewOrder &putClOrdId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    NewOrder &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::int8_t priceExponent() SBE_NOEXCEPT
    {
