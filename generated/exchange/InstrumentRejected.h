...
#ifndef _EXCHANGE_INSTRUMENTREJECTED_CXX_H_
#define _EXCHANGE_INSTRUMENTREJECTED_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(69);
    ...
    InstrumentRejected(char *buffer, const std::uint64_t bufferLength) :
        InstrumentRejected(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    InstrumentRejected(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        InstrumentRejected(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    InstrumentRejected &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    InstrumentRejected &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    InstrumentRejected &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        InstrumentRejected skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    InstrumentRejected &sessionId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    InstrumentRejected &senderCompId(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    InstrumentRejected &putSenderCompId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentRejected &putSenderCompId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentRejected &putSenderCompId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putSenderCompId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentRejected &putSenderCompId(const std::string_view str)
    {
    ...
    #else
    InstrumentRejected &putSenderCompId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    InstrumentRejected &account(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putAccount(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentRejected &putAccount(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentRejected &putAccount(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putAccount(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentRejected &putAccount(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #else
    InstrumentRejected &putAccount(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    InstrumentRejected &clOrdId(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putClOrdId(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentRejected &putClOrdId(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentRejected &putClOrdId(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putClOrdId(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentRejected &putClOrdId(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    #else
    InstrumentRejected &putClOrdId(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    InstrumentRejected &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    InstrumentRejected &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    InstrumentRejected &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 60, src, sizeof(char) * 8);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    InstrumentRejected &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    InstrumentRejected &putSymbol(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #else
    InstrumentRejected &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    InstrumentRejected &reason(const RejectReason::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const InstrumentRejected &_writer)
{
    InstrumentRejected writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "InstrumentRejected", )";
